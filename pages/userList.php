...
session_start();
    ...
    if(isset($_GET["forgive"]))
    {
        $conn=new mysqli('localhost:3306', USER, PASSWD, DBNAME);
        $conn->query("UPDATE users SET is_banned=0 WHERE id={$_GET["forgive"]};");
        $conn->close();
    }
    $conn=new mysqli('localhost:3306', USER, PASSWD, DBNAME);
                            ...
                            <table class="table user-list table-striped">
                                ...
                                <tr>
                                    ...
                                    <?php
                                        if ($part["is_banned"]==0)
                                        {
                                            ?><td><a href="?id='<?=$part["id"]?>'" class="btn btn-danger" role="button">Zbanuj</a></td><?php
                                        }
                                        else
                                        {
                                            ?><td><a href="?forgive='<?=$part["id"]?>'" class="btn btn-success" role="button">Odblokuj</a></td><?php
                                        }
                                ?></tr>
                                <?php } ?>
                            </table>
